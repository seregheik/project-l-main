...
> Image of a kid card showing the different sections. (A) Suit icon in the top left. (B) Public action box at the top. (C) Private action box in the middle. (D) Best Friend icon next to the character art. (E) Kid's nickname at the bottom.

1. **SUIT**: Every card has one of six suits or the wild coin suit. Some cards show two of the same suit or mix one suit with the wild coin suit.
...
> Image illustrating the anatomy of a player board featuring several key areas marked with letters A through G to guide players during their turns. At the very top, area A represents the Yard, where unused cards are placed for potential recruitment by rivals , while B is the Lookout for storing cards "forever" to permanently boost your actions. The top right features area C, the STUFF section, which acts as storage for up to four pizza and four toy tokens , complemented by area D, the Pack, which allows for additional resource storage that scales with your fort's progress. The bottom of the board is dedicated to the Fort Level track at area E , with area F detailing the resource costs required to advance and area G highlighting specific milestones where you gain special rewards like made-up rules, perks, or the macaroni sculpture. Centered on the board is a concise five-step Turn Summary to keep your neighborhood "posse" organized as you scramble for pizza and victory points
...
7. **PROMPT TO GAIN CARD**: When you reach Fort Level 1, 2, or 5, you get a made-up rule, perk, or macaroni sculpture (page 10).
...
Example: Muhammad plays Doodles and uses the private (bottom) action fully. He also wants to use the public (top) action. He wants to add one card with (icon of glue suit - depicted as a yellow-orange bottle of glue with a white tip) and one with (icon of coin suit - depicted as a round, multi-colored wild suit icon)(icon of crown suit - depicted as a light-blue crown with three points), using the wild (icon of coin suit) as (icon of glue suit). However, he can only hold two more toys in his STUFF, so adding both of those cards doesn’t do anything different from adding just one of them. Because of this, he can add either of these cards, but not both.

> The image shows a player (Muhammad) using the card "Doodles" to perform a public action to gather toys. This action is represented by a "GATHER RESOURCE" icon (a palm holding a tile) and a blue toy cube , followed by a "PER" icon (a bold X) and a glue suit. To improve the action, the player adds two cards: one with a glue suit and one with a wild coin suit. However, the player's "STUFF" board only has two empty slots for toys , meaning that adding both cards does not provide any additional benefit over adding just one. As a result, the player can only add one of these cards and gains two toy tokens , as indicated by the red arrow moving two cubes into the available hexagon slots on the board.
...
- **Take any card from the Park.**
  Immediately draw a new card from the Park deck to replace it.
- **Take any card from any rival player’s Yard.**
  They do not draw a new card.
- **Draw a card from the Park deck.**

### PHASE 4: DISCARD (Page 9)
...
Each player has a Fort, which starts at Level 0. You can increase your Fort Level by one with the Advance Fort action (page 12), spending the resources listed between your current Level and the next Level.
...
**When you reach Level 1**, privately choose a **made-up rule** card (page 15) from the central stack and put it in front of you facedown. Your made-up rule will score you points at the end of the game.

**When you reach Level 2**, choose a face-up **perk** card (page 16) from the central row and put it in front of you. Some perks have persistent effects that you will have for the entire game, while other perks can only be used once per game.
...
**GATHER RESOURCE** (icon of a palm with a tile on it)
...
**TRASH THIS CARD** (icon of a card being ripped in half with a circular arrow)
Once you finish this action and any followers have finished their actions, return this card to the box.

**RECRUIT CARD** (icon of a card with a plus sign inside)
Recruit one card exactly as described in **Recruit** (page 9).

**ADVANCE FORT** (icon of a "Fort" flag with a plus sign)
Pay the resources shown between your current and next Fort Level, returning them to the shared supply. You may pay from your STUFF, Pack, or both. Then, move your fort level marker to the next Level. Your new Level may prompt you to gain a card, as described in **Fort Level** (page 10).

**SCORE POINT** (icon of "VP" inside a laurel wreath)
Advance your score marker once on the Victory Track.

**SPEND RESOURCE** (icon of a resource tile with an "X" over it)
Remove the resource shown from your STUFF or Pack, returning it to the shared supply.

**TRASH RIVAL CARD** (icon of a grumpy face next to a ripped card)
Return one card from any rival player's Yard to the box.

**COPY PACK** (icon of a backpack with a palm underneath)
Gain the same resources currently in your Pack and add them to your STUFF.

**COPY RIVAL PACK** (icon of a grumpy face next to a backpack with a palm underneath)
Gain the same resources currently in any rival player's Pack and add them to your STUFF. (Y*ou don't steal them!*)

**CONVERT RESOURCE** (icon of pizza and toys with arrows pointing between them)
...
**CARDS IN YOUR LOOKOUT** (icon of a card tucked behind a tree)
To clarify, this is the number of cards, not suits.

**MODIFIED COST TO ADVANCE FORT** (icon of hexagons containing -1 and +1)
This Advance Fort action costs one less or one more resource, as shown, of either type.

**PER** (icon of a large bold X)
...
> Two image illustrations of the PER action symbol in the game Fort. The first example shows the Gather Resource icon followed by the Per icon and the Glue suit icon, and a caption "**Gather toys equal to glue.**" which indicates the player should gather toys equal to their number of glue symbols. The second example features the Score Point icon followed by the Per icon and the Resources in your Pack icon with the caption "**Score victory points equal to number of resources in your Pack.**" These symbols follow the rule that the action on the left is performed a number of times determined by the symbol or suit on the right.

**THEN** (icon of a solid right-pointing triangle)
...
> The THEN action symbol, represented by a solid right-pointing triangle, indicates a sequence where the action on the left must be completed before the action on the right can be performed. In the first example, the Trash Rival Card icon is followed by the Gather Resource icon, directing the player to trash one card from a rival player's yard and then gather one resource and with a caption "**Trash one card from a rival player's Yard, then gather one resource.**". The second example shows the Advance Fort icon followed by two Score Point icons and a caption "**Advance your Fort Level, paying its cost, then score 2 victory points**" requiring the player to pay the cost to advance their fort level before scoring two victory points.

**PARENTHESES** (icon of open and close parentheses symbols)
Parentheses contain a group of actions and symbols. When a group is multiplied by an x sign, do everything inside the parentheses the number of times shown by the symbol on the right side of the x sign.

> The provided image features two complex examples of the PARENTHESES symbol, which groups actions to be multiplied by the PER symbol. In the first example, the Recruit Card icon and the Gather Resource icon are enclosed in parentheses and followed by PER symbol and the Squirtgun Suit and caption "**Recruit one card, then gather one toy, equal to squirtguns. (Each time, you must recruit in order to gather the toy.)**", meaning the player recruits one card and then gathers one toy for every squirtgun symbol they have. The second example groups a Score Point icon multiplied by the Book Suit within parentheses, followed by the THEN symbol and the Trash This Card icon and caption "**Score victory points equal to books. then trash this card.**", This sequence instructs the player to score victory points equal to their number of books and then permanently return that card to the box.

## GLOSSARY: MADE-UP RULES (Page 15)
...
**Friendship Bracelet**: At the end of the game, score 1 point for each different suit you have, except the coin (icon of coin suit) suit. If you have all six suits, score an extra 1 point .
...
**Play Palace**: At the end of the game, score 1 point per crown (icon of crown suit) suit you have, not counting coins (icon of coin suit).
...
**Slime Lab**: At the end of the game, score 1 point per book (icon of book suit) suit you have, not counting coins (icon of coin suit).

## GLOSSARY: PERKS (Page 16)
